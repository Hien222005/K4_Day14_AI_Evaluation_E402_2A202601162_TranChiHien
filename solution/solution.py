...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_lines}\n"
            "Score each criterion from 0.0 to 1.0. Respond as a JSON object "
            "mapping each criterion name to its score."
        )
        response = self.judge_llm_fn(prompt)
        try:
            parsed = json.loads(response)
            scores = {str(name): float(score) for name, score in parsed.items()}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        first_position_avgs = []
        second_position_avgs = []
        for index, entry in enumerate(scores_batch):
            values = list(entry.get("scores", {}).values())
            if not values:
                continue
            entry_avg = sum(values) / len(values)
            if index % 2 == 0:
                first_position_avgs.append(entry_avg)
            else:
                second_position_avgs.append(entry_avg)

        positional_bias = False
        if first_position_avgs and second_position_avgs:
            first_mean = sum(first_position_avgs) / len(first_position_avgs)
            second_mean = sum(second_position_avgs) / len(second_position_avgs)
            positional_bias = first_mean > second_mean + 0.1
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_avg_faithfulness = _avg([r.faithfulness for r in new_results])
        new_avg_relevance = _avg([r.relevance for r in new_results])
        new_avg_completeness = _avg([r.completeness for r in new_results])

        baseline_avg_faithfulness = _avg([r.faithfulness for r in baseline_results])
        baseline_avg_relevance = _avg([r.relevance for r in baseline_results])
        baseline_avg_completeness = _avg([r.completeness for r in baseline_results])
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        lowest_metrics = [name for name, score in scores.items() if score == min_score]
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("off_topic"):
            suggestions.append(
                "Add few-shot examples showing complete answers to improve topic alignment"
            )
        ...
        while len(suggestions) < 3:
            suggestions.append(
                "Review full pipeline for the remaining failure cases and re-run benchmark"
            )
